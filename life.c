...
int main(int argc, char **argv)
{
  ...
  for(current_row = 1; current_row <= NUMBER_OF_ROWS;
      current_row++)
  {
#pragma omp parallel for private(current_column)
  ...
  for(current_time_step = 0; current_time_step <= NUMBER_OF_TIME_STEPS - 1;
      current_time_step++)
  {
#pragma omp parallel private(current_column)
...
#pragma omp parallel for private(current_column, neighbor_row, neighbor_column, number_of_alive_neighbors)
      for(current_column = 1; current_column <= NUMBER_OF_COLUMNS;
          current_column++)
      {
