...
#pragma omp parallel for private(i) reduction(+:num_infected)
...
#pragma omp parallel for private(i) reduction(+:num_susceptible)
...
#pragma omp parallel for private(person1, infected_nearby, person2) \
    ...
    /* For each person, spawn threads to do the following */
#pragma omp parallel for private(i) \
                         reduction(+:recovery_attempts) \
                         reduction(+:num_dead) \
                         reduction(+:num_infected) \
                         reduction(+:num_deaths) \
                         reduction(+:num_immune)
    for(i = 0; i < num_people; i++)
    {
